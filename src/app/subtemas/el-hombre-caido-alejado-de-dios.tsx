import React from 'react';

export default function ElHombreCaidoAlejadoDeDios() {
    return (
        <article id="clase" className="block min-h-screen">
            <div className="min-h-screen max-w-4xl mx-auto px-4 py-8">
                <h1 className="text-4xl font-bold text-yellow-400 mb-8">
                    El Hombre Caído Alejado de Dios
                </h1>
                {/* Objetivo */}
                <section className="mb-8">
                    <h2 className="text-2xl font-semibold text-white mb-4">Objetivo</h2>
                    <div className="bg-black/30 backdrop-blur-lg p-6 rounded-lg border border-white/10">
                        <p className="text-lg text-gray-200 leading-relaxed">
                            Analizar la transformación espiritual del ser humano desde la perspectiva bíblica, comprendiendo la caída y sus consecuencias, la oscuridad espiritual y su impacto en la identidad humana, y el proceso de restauración a través de la luz de Cristo, con el propósito de reconocer la importancia de la guía del Espíritu de Dios en la vida del creyente para alcanzar una existencia plena en comunión con Él.
                        </p>
                    </div>
                </section>
                {/* Bloques temáticos */}
                <section className="mb-8">
                    <h2 className="text-2xl font-semibold text-white mb-4">Bloques Temáticos</h2>
                    <div className="grid md:grid-cols-2 gap-4">
                        <div className="bg-black/20 p-4 rounded-lg">
                            <h3 className="text-yellow-400 font-semibold mb-2">La caída del ser humano y sus consecuencias</h3>
                            <ul className="list-disc list-inside text-gray-200 ml-4">
                                <li>La desobediencia de Adán y Eva y su impacto en la humanidad</li>
                                <li>La separación del hombre de la naturaleza divina</li>
                                <li>La realidad del pecado como decisión y ejercicio de la libertad</li>
                            </ul>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg">
                            <h3 className="text-yellow-400 font-semibold mb-2">La afectación del ser humano en su integridad</h3>
                            <ul className="list-disc list-inside text-gray-200 ml-4">
                                <li>Espíritu, alma y cuerpo en la antropología bíblica</li>
                                <li>La desconexión del espíritu humano con Dios</li>
                                <li>El puente entre el alma y el espíritu: el papel del corazón</li>
                                <li>El puente entre el alma y el cuerpo: el papel de la mente</li>
                            </ul>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg">
                            <h3 className="text-yellow-400 font-semibold mb-2">La oscuridad espiritual y sus manifestaciones</h3>
                            <ul className="list-disc list-inside text-gray-200 ml-4">
                                <li>El corazón entenebrecido y la mente reprobada</li>
                                <li>La idolatría y la necedad como consecuencias del rechazo a Dios</li>
                                <li>La degradación moral del ser humano sin Dios</li>
                                <li>La esclavitud del ser humano a la naturaleza carnal</li>
                            </ul>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg">
                            <h3 className="text-yellow-400 font-semibold mb-2">La transformación espiritual a través de la luz</h3>
                            <ul className="list-disc list-inside text-gray-200 ml-4">
                                <li>La Palabra de Dios como fuente de vida y restauración</li>
                                <li>Jesús, la Luz del mundo y su impacto en la existencia humana</li>
                                <li>La conversión como paso de la oscuridad a la luz</li>
                                <li>La nueva identidad en Cristo</li>
                            </ul>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg">
                            <h3 className="text-yellow-400 font-semibold mb-2">Pneumonomía vs. autonomía: El gobierno del Espíritu</h3>
                            <ul className="list-disc list-inside text-gray-200 ml-4">
                                <li>La propuesta bíblica de ser guiados por el Espíritu</li>
                                <li>La relación entre la voluntad humana y la voluntad de Dios</li>
                                <li>La vida en el Espíritu como alternativa a la vida en la carne</li>
                                <li>La transformación del entendimiento por la Palabra de Dios</li>
                            </ul>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg">
                            <h3 className="text-yellow-400 font-semibold mb-2">La batalla espiritual entre la luz y las tinieblas</h3>
                            <ul className="list-disc list-inside text-gray-200 ml-4">
                                <li>El gobierno de las tinieblas en el mundo sin Dios</li>
                                <li>La influencia de Satanás en el corazón entenebrecido</li>
                                <li>La victoria sobre las tinieblas a través de la luz de Cristo</li>
                                <li>La lucha espiritual y la resistencia en la fe</li>
                            </ul>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg md:col-span-2">
                            <h3 className="text-yellow-400 font-semibold mb-2">El destino final: Luz o tinieblas</h3>
                            <ul className="list-disc list-inside text-gray-200 ml-4">
                                <li>La elección entre la vida y la muerte espiritual</li>
                                <li>Las promesas de Dios para quienes caminan en la luz</li>
                                <li>La restauración final y el cumplimiento del propósito divino</li>
                                <li>La esperanza de la ciudadanía celestial en Cristo</li>
                            </ul>
                        </div>
                    </div>
                </section>
                {/* Desarrollo temático */}
                <section className="mb-8">
                    <h2 className="text-2xl font-semibold text-white mb-8">Desarrollo Temático</h2>
                    <div className="space-y-8">
                        {/* Aquí irán los bloques destacados y frases clave, citas bíblicas, resúmenes, siguiendo el patrón de los otros capítulos. */}
                        {/* Por ejemplo: */}
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;La carta del apóstol Pablo a los Romanos menciona en el capítulo 3 verso 23 que todos pecaron y se encuentran destituidos de la gloria de Dios.&quot;
                            </p>
                        </div>
                        <p className="text-lg text-gray-200 leading-relaxed">
                            En este capítulo vamos a entender la consecuencia de la caída de Adán y Eva para la humanidad. Es apenas lógico entender que si la humanidad participa de la naturaleza de Adán y Eva, pues arrastra consigo las consecuencias de su desobediencia en el paraíso, pero también de la promesa dada por Dios para su restauración hasta llegar al cumplimiento del propósito para el cual fuimos creados por El.
                        </p>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;El problema del hombre radica en su corazón.&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;Al separarse de Dios, el hombre sigue sus propios razonamientos, y por ello su corazón es ENTENEBRECIDO.&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;La Biblia no propone la autonomía sino la PNEUMONOMÍA, es decir, ser guiados por el Espíritu de Dios.&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;El espíritu es la vida, la luz, la conciencia de Dios en el hombre; es el canal de comunicación de Dios con el ser humano.&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;Cuando el entendimiento del ser humano está alejado de Dios, es decir de su Palabra o de su Espíritu, que es lo mismo, se verifica una muerte espiritual.&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;El ser humano sin Dios está entenebrecido su corazón y por lo tanto no permite que a su alma pase luz y vida, provocando la muerte espiritual (Efesios 2:1).&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;El texto bíblico dice que los hombres amaron más a las tinieblas que a la luz (Juan 1:12) haciendo referencia a Jesús como Luz del mundo (Juan 8:12).&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;El apóstol Pablo escribe a los Efesios: &apos;Y El os dio vida a vosotros, cuando estabais muertos en vuestros delitos y pecados...&apos; (Efesios 2:1-2).&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;El libro de los Proverbios dice que &apos;sobre toda cosa guardada, guarda tu corazón porque de él mana la vida&apos; (Proverbios 4:23).&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;La Palabra de Jesús dan espíritu y vida al ser humano (Juan 6:63).&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;El apóstol Pablo en la carta a los Efesios dice que &apos;en otro tiempo erais tinieblas, mas ahora sois luz en el Señor; andad como hijos de luz&apos; (Efesios 5:8).&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;El mundo de la tiniebla es el hombre sin luz o sin Dios, que es lo mismo.&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;El final del ser humano, más allá de la muerte física, es luz o tiniebla: luz para los que se dejaron guiar de la luz o la Palabra de Dios, tinieblas para aquellos que la rechazaron (Mateo 8:12).&quot;
                            </p>
                        </div>
                        <div className="bg-black/20 p-4 rounded-lg border border-white/10">
                            <p className="text-lg text-yellow-200 italic">
                                &quot;Pero hay una gran noticia revelada como promesa en las Palabras de Jesús cuando dice: &apos;...Yo soy la luz del mundo; el que me sigue, no andará en tinieblas, sino que tendrá la luz de la vida&apos; (Juan 8:12).&quot;
                            </p>
                        </div>
                        {/* Puedes seguir agregando más bloques destacados según lo desees */}
                    </div>
                </section>
            </div>
        </article>
    );
}